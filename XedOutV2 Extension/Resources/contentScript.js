        ...
        case 10:
          ...
          return new Promise(function (resolve) {
            chrome.runtime.sendMessage({
              ...
              console.log('[X Filter] Background response:', response);
              if (chrome.runtime.lastError) {
                console.error('[X Filter] Error sending message to background:', chrome.runtime.lastError);
                resolve(false);
              } else if (response && response.success) {
                resolve(response.result);
              } else {
                console.error('[X Filter] Background script error:', response ? response.error : 'Unknown error');
                resolve(false);
              }
            });
          });
